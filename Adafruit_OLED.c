...
// OLED SPI Pins
static const PinSetting SPIDC = { .port = GPIOA0_BASE, .pin = 0x80};    //62
static const PinSetting SPIR = { .port = GPIOA0_BASE, .pin = 0x10};     //59
static const PinSetting OLEDCD = { .port = GPIOA0_BASE, .pin = 0x40};   //61
...
*/
    unsigned long dummy;
        ...
        GPIOPinWrite(SPIDC.port, SPIDC.pin, 0);

        // Set OC (CS) low to select OLED
        GPIOPinWrite(OLEDCD.port, OLEDCD.pin, 0);
        ...
        GPIOPinWrite(OLEDCD.port, OLEDCD.pin, OLEDCD.pin);
...
*/
    unsigned long dummy;
        ...
        GPIOPinWrite(SPIDC.port, SPIDC.pin, SPIDC.pin);
...
*/
  ...
  GPIOPinWrite(SPIR.port, SPIR.pin, 0);   // RESET = RESET_LOW

  for(delay=0; delay<100; delay=delay+1);// delay minimum 100 ns

  GPIOPinWrite(SPIR.port, SPIR.pin, SPIR.pin);    // RESET = RESET_HIGH
